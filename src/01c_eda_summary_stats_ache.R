...
## summary stats, normality tests, and check for outliers for ache

Summarize(ache_ug_min_mg ~ treatment, data=rvm_gsh, digits=3)
#treatment  n  mean    sd   min    Q1 median    Q3   max
#1       24D 11 0.002 0.001 0.001 0.002  0.002 0.003 0.005
#2         C 11 0.002 0.001 0.001 0.001  0.002 0.003 0.004
#3       CPS 11 0.002 0.001 0.001 0.001  0.002 0.003 0.003
...
aggregate(ache_ug_min_mg ~ treatment,
          data = rvm_gsh,
          FUN = function(x) {y <- shapiro.test(x); c(y$statistic, y$p.value)})
#treatment ache_ug_min_mg.W ache_ug_min_mg.V2
#1       24D       0.85339065        0.04724944
#2         C       0.92741050        0.38525474
#3       CPS       0.85761485        0.05362138


rvm_gsh %>%
  group_by(treatment) %>%
  identify_outliers("ache_ug_min_mg")
## A tibble: 2 × 14
#treatment salamander_id weight_g svl_mm m_f   gsh_1_5_dilution_nM_mL gsh_1_8_dilution_n… ache_ug_min_mg log_gsh_15 log_gsh_18
#<fct>     <fct>            <dbl>  <dbl> <fct>                  <dbl>               <dbl>          <dbl>      <dbl>      <dbl>
#  1 24D       DS4               19.6   90.4 M                      18.3                22.8         0.00430       2.91       3.13
#2 24D       DS7               17.8   87.2 M                       4.69                5.83        0.00484       1.55       1.76
## … with 4 more variables: gsh_nM_mL <dbl>, log_gsh_158 <dbl>, is.outlier <lgl>, is.extreme <lgl>

outliers_ache <- c(which(rvm_gsh$salamander_id=='DS4'),which(rvm_gsh$salamander_id=='DS7'))
