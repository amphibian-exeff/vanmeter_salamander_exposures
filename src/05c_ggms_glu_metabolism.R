...
#ggm(rvm_glu[,3:14], methods = c("glasso"), community = TRUE,
...
corrplot(glu_cormatrix_cpf, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(glu_cormatrix_cpf, graph = "glasso", sampleSize = (n_samples_cpf),
       layout = "spring", refit = FALSE, title = "Original")

# 24d corrplot and qgraph FAILS
glu_cormatrix_d <- cor_auto(rvm_glu_std_d) #, forcePD = T
glu_cormatrix_d <- as.matrix(forceSymmetric(glu_cormatrix_d))
is.positive.definite(glu_cormatrix_d, tol=1e-8)
corrplot(glu_cormatrix_d, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(glu_cormatrix_d, graph = "glasso", sampleSize = (n_samples_d),
       layout = "spring", refit = FALSE, title = "Original")

# trying with 24d and covariance
glu_cov_d <- cov(rvm_glu_std_d)
glu_cov_d <- as.matrix(forceSymmetric(glu_cov_d))
qgraph(glu_cov_d, graph = "glasso", sampleSize = (n_samples_d),
       layout = "spring", refit = FALSE, title = "Original")
