...
p4 <- rvm_gsh %>%
  mutate(treatment = fct_relevel(treatment, 
    ...
    ggplot(aes(x=treatment, y=gsh_1_8_dilution_nM_mL, fill=treatment)) +
      geom_boxplot() +
      geom_jitter(shape=16, position=position_jitter(0.2)) +
      xlab("Treatment") + ylab("gsh_1_8_dilution_nM_mL") + ggtitle("Salamander gsh 1:8 dilution") +
      theme_bw()
p4

#combined figure jpg
gsh_combined <- ggarrange(p3, p4, heights = c(4, 4), widths=c(3.7,3.3),
                              labels = c("A", "B"),
                              ncol = 2, nrow = 1)
gsh_combined
...
# normality tests
aggregate(formula = gsh_1_5_dilution_nM_mL ~ treatment,
          data = rvm_gsh,
...
aggregate(formula = gsh_1_8_dilution_nM_mL ~ treatment,
          data = rvm_gsh,
          FUN = function(x) {y <- shapiro.test(x); c(y$statistic, y$p.value)})
...
aggregate(formula = log(gsh_1_5_dilution_nM_mL) ~ treatment,
          data = rvm_gsh,
          FUN = function(x) {y <- shapiro.test(x); c(y$statistic, y$p.value)})
...
aggregate(formula = log(gsh_1_8_dilution_nM_mL) ~ treatment,
          data = rvm_gsh,
          FUN = function(x) {y <- shapiro.test(x); c(y$statistic, y$p.value)})
