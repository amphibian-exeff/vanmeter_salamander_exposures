...
dim(rvm_glu_ala_mat)
ncols_glu_ala <- dim(rvm_glu_ala_mat)[[2]]
#View(rvm_glu_ala_mat)
rvm_glu_ala_mat_log <- log2(rvm_glu_ala_mat)

#View(rvm_glu_ala_mat_log)
# standardize is in 00support_functions.R
rvm_glu_ala_std = normal_score_transform(rvm_glu_ala_mat_log, 1:ncols_glu_ala)
#View(rvm_glu_ala_std) # standardized after log2 transformation

rvm_glu_ala_std_control <- rvm_glu_ala_std[1:11,]
#View(rvm_glu_ala_std_control)
rvm_glu_ala_std_cpf <- rvm_glu_ala_std[12:22,]
#View(rvm_glu_ala_std_cpf)
rvm_glu_ala_std_d <- rvm_glu_ala_std[23:33,]
#View(rvm_glu_ala_std_d)
rvm_glu_ala_std_treatment <- rvm_glu_ala_std[12:33,]
#View(rvm_glu_ala_std_treatment)
...
rvm_glu_ala_gg_wide <- as.data.frame(rvm_glu_ala_gg_wide)
#View(rvm_glu_ala_gg_wide)
colnames(rvm_glu_ala_gg_wide)[1] <- "treatment"
rvm_glu_ala_gg_wide$treatment <- as.factor(as.character(rvm_glu_ala_gg_wide$treatment)) 
#View(rvm_glu_ala_gg_wide)
...
pairs(rvm_glu_ala_std_control)
pairs(rvm_glu_ala_std_cpf)
pairs(rvm_glu_ala_std_24d)
...
glu_ala_cov <- cov(rvm_glu_ala_std)
#glu_ala_cormatrix <- var(rvm_glu_ala_std)
#glu_ala_cormatrix <- as.matrix(forceSymmetric(glu_ala_cormatrix))
# check if the matrix is positive definite or not
is.positive.definite(glu_ala_cov, tol=1e-8)
eigen(glu_ala_cov)$values # sll should be positive for positive definite
corrplot(glu_ala_cov, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45, diag=F)
# uses glasso
glu_ala_glasso <- qgraph(glu_ala_cov, graph = "glasso", sampleSize = n_samples_all,
                         layout = "spring", refit = FALSE, title = "glu_ala", threshold=T)
plot(glu_ala_glasso)
...
glasso_fit <- glasso(glu_ala_cov, rho=0)
# Extract estimated adjacency matrix from the glasso_fit
adj_matrix <- glasso_fit$wi
# Plot estimated network
glu_ala_qgraph_adjmatrix_cov <- qgraph(adj_matrix, layout = "spring", labels = colnames(glu_ala_cormatrix))
plot(glu_ala_qgraph_adjmatrix_cov)
#library(patchwork)
#(plot(glu_ala_glasso) | plot(glu_ala_glasso))/(plot(glu_ala_qgraph_adjmatrix_cov))
#library(cowplot)
#plot_grid(plot(glu_ala_glasso), plot(glu_ala_glasso), plot(glu_ala_glasso), ncol = 3)
...
eigen(glu_ala_cormatrix_control)$values # sll should be positive for positive definite
corrplot(glu_ala_cormatrix_control, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
# control covariance
glu_ala_cov_control <- cov(rvm_glu_ala_std_control)
is.positive.definite(glu_ala_cov_control, tol=1e-8)
eigen(glu_ala_cov_control)$values # sll should be positive for positive definite
corrplot(glu_ala_cov_control, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)

qgraph(glu_ala_cov_control, graph = "glasso", sampleSize = n_samples_control,
...
glu_ala_cormatrix_treatment <- var(rvm_glu_ala_std_treatment)
...
qgraph(glu_ala_cormatrix_treatment, graph = "glasso", sampleSize = (n_samples_cpf + n_samples_d),
       layout = "spring", refit = FALSE, title = "glu_ala-All treatments")

#chlorpyrifos corrplot and qgraph FAILS
glu_ala_cormatrix_cpf <- cor_auto(rvm_glu_ala_std_cpf) #, forcePD = T
glu_ala_cormatrix_cpf <- var(rvm_glu_ala_std_cpf) #, forcePD = T
...
epsilon <- 0.0001
glu_ala_cormatrix
glu_ala_cormatrix_ridge <- glu_ala_cormatrix + diag(rep(epsilon, nrow(glu_ala_cormatrix)))
eigen(glu_ala_cormatrix_ridge)$values #its only the upper triangular
is.positive.definite(glu_ala_cormatrix_ridge, tol=1e-8)
...
glu_ala_cormatrix_chol <- chol(glu_ala_cormatrix)
is.positive.definite(as.matrix(glu_ala_cormatrix_chol), tol=1e-8)
# make symmetric (machine precision)
glu_ala_cormatrix_chol[lower.tri(glu_ala_cormatrix_chol)] = t(glu_ala_cormatrix_chol)[lower.tri(glu_ala_cormatrix_chol)]
glu_ala_cormatrix_chol
dim(glu_ala_cormatrix_chol)
isSymmetric.matrix(glu_ala_cormatrix_chol)
eigen(glu_ala_cormatrix_chol)$values
is.positive.definite(as.matrix(glu_ala_cormatrix_chol), tol=1e-8)
...
glu_ala_glasso <- EBICglasso(glu_ala_cormatrix_chol, n_samples_all, gamma = 0.5, penalize.diagonal = FALSE, nlambda = 100, 
