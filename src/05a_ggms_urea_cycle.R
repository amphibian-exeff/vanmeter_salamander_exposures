...
#ggm(rvm_urea[,3:14], methods = c("glasso"), community = TRUE,
...
# paired scatterplots
pairs(rvm_urea_std, col=rvm_urea$treatment)
pairs(rvm_urea_std_control)
pairs(rvm_urea_std_cpf)
pairs(rvm_urea_std_24d)

#for everything, we don't have sample sizes to do treatments
n_samples_all <- nrow(rvm_urea_std)
n_samples_control <- nrow(rvm_urea_std_control)
n_samples_cpf <- nrow(rvm_urea_std_cpf)
n_samples_d <- nrow(rvm_urea_std_d)
...
urea_cormatrix <- cor_auto(rvm_urea_std)
urea_cov <- cov(rvm_urea_std)
#urea_cormatrix <- var(rvm_urea_std)
#urea_cormatrix <- as.matrix(forceSymmetric(urea_cormatrix))
# check if the matrix is positive definite or not
is.positive.definite(urea_cov, tol=1e-8)
eigen(urea_cov)$values # sll should be positive for positive definite
corrplot(urea_cov, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45, diag=F)
# uses glasso
urea_glasso <- qgraph(urea_cov, graph = "glasso", sampleSize = n_samples_all,
       layout = "spring", refit = FALSE, title = "Urea", threshold=T)
plot(urea_glasso)
...
glasso_fit <- glasso(urea_cov, rho=0)
# Extract estimated adjacency matrix from the glasso_fit
adj_matrix <- glasso_fit$wi
# Plot estimated network
urea_qgraph_adjmatrix_cov <- qgraph(adj_matrix, layout = "spring", labels = colnames(urea_cormatrix))
plot(urea_qgraph_adjmatrix_cov)
#library(patchwork)
#(plot(urea_glasso) | plot(urea_glasso))/(plot(urea_qgraph_adjmatrix_cov))
#library(cowplot)
#plot_grid(plot(urea_glasso), plot(urea_glasso), plot(urea_glasso), ncol = 3)

### controls only
# control corrplot works but qgraph FAILS
# control correlation
urea_cormatrix_control <- cor_auto(rvm_urea_std_control)
is.positive.definite(urea_cormatrix_control, tol=1e-8)
eigen(urea_cormatrix_control)$values # sll should be positive for positive definite
corrplot(urea_cormatrix_control, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
# control covariance
urea_cov_control <- cov(rvm_urea_std_control)
is.positive.definite(urea_cov_control, tol=1e-8)
eigen(urea_cov_control)$values # sll should be positive for positive definite
corrplot(urea_cov_control, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)

qgraph(urea_cov_control, graph = "glasso", sampleSize = n_samples_control,
...
#combined treatments
urea_cormatrix_treatment <- cor_auto(rvm_urea_std_treatment, forcePD = T)
urea_cormatrix_treatment <- var(rvm_urea_std_treatment)
urea_cormatrix_treatment <- as.matrix(forceSymmetric(urea_cormatrix_treatment))
is.positive.definite(urea_cormatrix_treatment, tol=1e-8)
corrplot(urea_cormatrix_treatment, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_cormatrix_treatment, graph = "glasso", sampleSize = (n_samples_cpf + n_samples_d),
       layout = "spring", refit = FALSE, title = "Urea-All treatments")

#chlorpyrifos corrplot and qgraph FAILS
urea_cormatrix_cpf <- cor_auto(rvm_urea_std_cpf) #, forcePD = T
urea_cormatrix_cpf <- var(rvm_urea_std_cpf) #, forcePD = T
urea_cormatrix_cpf <- as.matrix(forceSymmetric(urea_cormatrix_cpf))
is.positive.definite(urea_cormatrix_cpf, tol=1e-8)
corrplot(urea_cormatrix_cpf, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_cormatrix_cpf, graph = "glasso", sampleSize = (n_samples_cpf),
       layout = "spring", refit = FALSE, title = "Original")

# 24d corrplot and qgraph FAILS
urea_cormatrix_d <- cor_auto(rvm_urea_std_d) #, forcePD = T
urea_cormatrix_d <- as.matrix(forceSymmetric(urea_cormatrix_d))
is.positive.definite(urea_cormatrix_d, tol=1e-8)
corrplot(urea_cormatrix_d, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_cormatrix_d, graph = "glasso", sampleSize = (n_samples_d),
       layout = "spring", refit = FALSE, title = "Original")

# trying with 24d and covariance
urea_cov_d <- cov(rvm_urea_std_d)
urea_cov_d <- as.matrix(forceSymmetric(urea_cov_d))
qgraph(urea_cov_d, graph = "glasso", sampleSize = (n_samples_d),
       layout = "spring", refit = FALSE, title = "Original")
...
epsilon <- 0.0001
urea_cormatrix
urea_cormatrix_ridge <- urea_cormatrix + diag(rep(epsilon, nrow(urea_cormatrix)))
eigen(urea_cormatrix_ridge)$values #its only the upper triangular
is.positive.definite(urea_cormatrix_ridge, tol=1e-8)
...
urea_cormatrix_chol <- chol(urea_cormatrix)
is.positive.definite(as.matrix(urea_cormatrix_chol), tol=1e-8)
# make symmetric (machine precision)
urea_cormatrix_chol[lower.tri(urea_cormatrix_chol)] = t(urea_cormatrix_chol)[lower.tri(urea_cormatrix_chol)]
urea_cormatrix_chol
dim(urea_cormatrix_chol)
isSymmetric.matrix(urea_cormatrix_chol)
eigen(urea_cormatrix_chol)$values
is.positive.definite(as.matrix(urea_cormatrix_chol), tol=1e-8)
...
urea_glasso <- EBICglasso(urea_cormatrix_chol, n_samples_all, gamma = 0.5, penalize.diagonal = FALSE, nlambda = 100, 
           lambda.min.ratio = 0.01, returnAllResults = FALSE, checkPD = TRUE, 
           countDiagonal = FALSE, refit = FALSE, threshold = FALSE,
           verbose = TRUE)
corrplot(urea_glasso, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_glasso, graph = "glasso", sampleSize = n_samples_all,
       layout = "spring", refit = FALSE, title = "Original")
...
is.positive.definite(urea_cormatrix_control, tol=1e-8)
# check if the matrix is positive definite or not
eigen(urea_cormatrix_control)$values
# Add the constant to the diagonal of the matrix
epsilon <- 0.0001
urea_cormatrix_control <- urea_cormatrix_control + diag(rep(epsilon, nrow(urea_cormatrix_control)))
eigen(urea_cormatrix_control)$values
is.positive.definite(urea_cormatrix_control, tol=1e-8)


urea_glasso_control <- EBICglasso(urea_cormatrix_control, n_samples_control, gamma = 0.5, penalize.diagonal = FALSE, nlambda = 100, 
                          lambda.min.ratio = 20, returnAllResults = FALSE, checkPD = TRUE, 
                          countDiagonal = FALSE, refit = FALSE, threshold = FALSE,
                          verbose = TRUE)
corrplot(urea_glasso_control, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_glasso_control, graph = "glasso", sampleSize = n_samples_all,
       layout = "spring", refit = FALSE, title = "Original")
...
is.positive.definite(urea_cormatrix_cpf, tol=1e-8)
# check if the matrix is positive definite or not
eigen(urea_cormatrix_cpf)$values
# Add the constant to the diagonal of the matrix
epsilon <- 0.0001
urea_cormatrix_cpf <- urea_cormatrix_cpf + diag(rep(epsilon, nrow(urea_cormatrix_cpf)))
eigen(urea_cormatrix_cpf)$values
is.positive.definite(urea_cormatrix_cpf, tol=1e-8)
urea_glasso_cpf <- EBICglasso(urea_cormatrix_cpf, n_samples_cpf, gamma = 0.5, penalize.diagonal = FALSE, nlambda = 100, 
                                  lambda.min.ratio = 0.01, returnAllResults = FALSE, checkPD = TRUE, 
                                  countDiagonal = FALSE, refit = FALSE, threshold = FALSE,
                                  verbose = TRUE)
corrplot(urea_glasso_cpf, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
qgraph(urea_glasso_cpf, graph = "glasso", sampleSize = n_samples_all,
       layout = "spring", refit = FALSE, title = "Original")

# 24d
urea_glasso_d <- EBICglasso(urea_cormatrix_d, n_samples_d, gamma = 0.5, penalize.diagonal = FALSE, nlambda = 100, 
                                  lambda.min.ratio = 0.01, returnAllResults = FALSE, checkPD = TRUE, 
                                  countDiagonal = FALSE, refit = FALSE, threshold = FALSE,
                                  verbose = TRUE)
corrplot(urea_glasso_d, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
